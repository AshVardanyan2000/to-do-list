...
const ListItem: FC<IListItemProps> = ({task}) => {
    ...
    const onSeeMoreClick: () => void = () => {
        ...
        setTextClassName(prev => !openText
            ? [...prev, classes.task_show_more]
            : prev.filter(p => p !== classes.task_show_more))
    }
        ...
        <div className={classes.task} ref={itemRef} style={{background: task.type === 'done' ? '#AEF5A6' : '#FFEFC2'}}>
            ...
            <div className={classes.task_actions}>
                {task.type !== 'done' && <div
                    ...
                    role='button'
                    tabIndex={0}
                    onClick={(e: React.MouseEvent<HTMLDivElement>) => actionClick(e, task.type !== 'important' ? 'important' : 'undone')}
                >
                ...
                >
                    ...
                    {openModal && (
                        <div className={classes.action_modal_wrapper}>
                            {actionModalItems.filter((a: IActionItem) => task.type === 'done'
                                ? a.path !== 'done' : a.path !== 'undone').map((a: IActionItem) => (
                                <div
